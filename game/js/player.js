/**
 * player.js — Player character and Bullet projectile (unchanged).
 */
...
export class Player {
    constructor(x, y) {
        ...
        this.dashDx       = 0;
        this.dashDy       = 0;
        this._kbX = 0; this._kbY = 0; this._kbTimer = 0; this._KB_DUR = 220;
        this.hurtFlash  = 0;
        this.invincible = 0;
        this.dead       = false;
    }
    ...
    update(input, delta) {
        ...
        if (this._kbTimer > 0) {
            ...
            if (input.dashPressed && this.dashCooldown <= 0) {
                let ddx, ddy;
                if (mag > 0) { ddx = dx / mag; ddy = dy / mag; }
                else {
                    const mx = input.mouseX - this.x, my = input.mouseY - this.y;
                    const mm = Math.hypot(mx, my);
                    if (mm > 0) { ddx = mx / mm; ddy = my / mm; }
                    else { ddx = 1; ddy = 0; }
                }
    ...
    applyKnockback(dx, dy) {
        this._kbX     = dx / (this._KB_DUR / 1000);
